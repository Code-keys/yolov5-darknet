...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    pr_score = 0.01  # score to evaluate P and R https://github.com/ultralytics/yolov3/issues/898
...
def compute_ap(recall, precision, method='continuous'):
...
def compute_lost_KD(output_s, output_t, num_classes, batch_size, T=3.0): 
    Lambda_ST = 0.001
...
def compute_lost_KD7( output_s, output_t, prune_idx, T = 2.0):

    criterion_st = torch.nn.KLDivLoss(reduction='sum') 
    lfeature = 0

    ft = torch.cuda.FloatTensor if output_s[0].is_cuda else torch.Tensor
    lcls, lbox, lobj, lfeature = ft([0]), ft([0]), ft([0]), ft([0])

    assert output_s.__len__() == output_t.__len__()
    for idx in range(output_s.__len__()): 
        ft_s, ft_t  = output_s[idx],  output_t[idx]
        if idx in prune_idx or ft_s is [] :
            continue
        if ft_s.size(1) != ft_t.size(1):
            continue
        batch_size, ch = ft_s.size(0), ft_s.size(1)
        ft_s = ft_s.view(-1, ch )
        ft_t = ft_t.view(-1, ch )
        lfeature += criterion_st(nn.functional.log_softmax(ft_s / T, dim=1),
                                nn.functional.softmax(ft_t / T, dim=1)) * (T * T) / batch_size
    return lfeature
