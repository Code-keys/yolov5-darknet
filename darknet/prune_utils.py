...
def get_input_mask(module_defs, idx, CBLidx2mask):
    ...
    # for mobilenet
        ...
        if len(route_in_idxs) == 1:
            #my
            if module_defs[route_in_idxs[0]]['type'] != 'convolutional':
                return get_input_mask(module_defs, idx - 1, CBLidx2mask)
            else:
                mask = CBLidx2mask[route_in_idxs[0]]
                if 'groups' in module_defs[idx - 1]:
                    groups = int(module_defs[idx-1]['groups'])
                    group_id = int(module_defs[idx-1]['group_id'])
                    return mask[(group_id * mask.shape[0] // groups): ((group_id + 1) * mask.shape[0] // groups)]
                return mask
            ...
            if module_defs[route_in_idxs[1] - 1]['type'] == 'maxpool':
                ...
                if module_defs[route_in_idxs[0]]['type'] != 'convolutional':
                    mask1 = get_input_mask(module_defs, route_in_idxs[0], CBLidx2mask)
                else:
                    mask1 = CBLidx2mask[route_in_idxs[0]]
                if module_defs[route_in_idxs[1]]['type'] != 'convolutional':
                    mask2 = get_input_mask(module_defs, route_in_idxs[1], CBLidx2mask)
                else:
...
def init_weights_from_loose_model(compact_model, loose_model, CBL_idx, Other_idx, CBLidx2mask):
    for idx in CBL_idx:
        ...
        if 'groups' in loose_model.module_defs[idx]:
            groups = int(loose_model.module_defs[idx]['groups'])
            tmp = loose_conv.weight.data[:, 0:int(len(in_channel_idx)/groups), :, :].clone()
        else:
        ...
        if model_def['type'] == 'convolutional' or model_def['type'] == 'depthwise' or model_def['type'] == 'se':
            ...
            if len(from_layers) == 1:
                ...
                if 'groups' in model_def:
                    groups = int(model_def['groups'])
                    group_id = int(model_def['group_id'])
                    activation = activation[(group_id * activation.shape[0] // groups): ((group_id+1) * activation.shape[0] // groups)]
                update_activation(i, pruned_model, activation, CBL_idx)
            elif len(from_layers) == 2:
                actv1 = activations[i + from_layers[0] if from_layers[0] < 0 else from_layers[0]]
...
def obtain_bn_mask(bn_module, thre):
    ...
    while True:
        thre1 = thre2.cuda()
        mask = bn_module.weight.data.abs().ge(thre1).float()
        mask1 = mask.cpu().numpy()
        remain = int(mask1.sum())
        if remain > 0:
            break
        else:
            thre2 = thre2 * 1.1
