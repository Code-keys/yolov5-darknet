...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    shutil.copyfile( opt.cfg , save_dir / "model.cfg" )
    ...
    if weights.endswith('.pt'):  # pytorch format 
        ...
        try:
            ...
            new_ckpt = {'optimizer' :None, 'epoch' :0 }
            new_ckpt['model'] = {
                k: v for k, v in ckpt['model'].items() 
                if model.state_dict()[k].numel() == v.numel()
                }
            model.load_state_dict(new_ckpt['model'], strict=False)
    ...
    if opt.steplr == 1 :
        scheduler = lr_scheduler.StepLR(optimizer, step_size= ( epochs+50  ) // 3 , gamma=0.2 )
        # scheduler = lr_scheduler.MultiStepLR(optimizer, step_size??= [ (epochs+50  ) // 3, (epochs+50  ) *2// 3   ] , gamma=0.2 )
    else :
    ...
    if rank in [-1, 0]:
        ...
        # testloader = create_dataloader(test_path, (imgsz_test // 64 )*64 , total_batch_size, gs, opt,  # testloader
        #                                hyp=hyp, cache=opt.cache_images and not opt.notest, rect=True,
        #                                rank=-1, world_size=opt.world_size, workers=opt.workers, pad=0.5)[0]
        testloader = create_dataloader(test_path, imgsz_test, total_batch_size, 64 , opt, pad=0.5, rect=True)[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hyp', type=str, default='darknet/prune.yaml', help='hyperparameters path')
    ...
    parser.add_argument('--name', default='cfg-Prune', help='save to project/name')
    parser.add_argument('--steplr',type=int , default=0, help='save to project/name') 
    ...
    opt.name = opt.name+"_adam" if opt.adam else  opt.name
