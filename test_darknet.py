...
from darknet import Darknet ,   load_darknet_weights, save_weights
...
def test(data,
    ...
    if training:  # called by train.py
        ...
        # Model
        # pretrained = save_weights.endswith('.pt') 
        if weights.endswith('.pt'):  # pytorch format 
            try:
                ...
                model.load_state_dict(new_ckpt['model'].state_dict() , strict=False)
                print('Transferred %g/%g items from %s' % (len(new_ckpt['model'].items()), len(model.state_dict()), weights))  # report
            save_weights(model, save_weights.replace(".pt", '.weights') )
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            if save_json:
                ...
                image_id =  path.stem # int(path.stem) if path.stem.isnumeric() else path.stem
    ...
    if save_json and len(jdict):
        ...
        anno_json = opt.json    # annotations json
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg',  type=str, default='/home/gy/CX/yolov5-4.0/darknet/yolov5sm-l/cfg-Prune/model.cfg', help='model.pt path(s)')
    parser.add_argument('--weights', type=str, default='/home/gy/CX/yolov5-4.0/darknet/yolov5sm-l/cfg-Prune/weights/last.weights', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='/home/gy/CX/VisDrone/VisDrone.yaml', help='*.data path')
    parser.add_argument('--batch-size', type=int, default=4, help='size of each image batch')
    parser.add_argument('--img-size', type=int, default=1536, help='inference size (pixels)')
    parser.add_argument('--conf-thres', type=float, default=0.001, help='object confidence threshold')
    parser.add_argument('--iou-thres', type=float, default=0.5, help='IOU threshol d for NMS')
    ...
    parser.add_argument('--json', type=str ,default="/home/gy/CX/VisDrone/val/gt.json", help='save a cocoapi-compatible JSON results file')

    parser.add_argument('--project', default='runs/test', help='save to project/name')
    parser.add_argument('--name', default='exp', help='save to project/name')
    parser.add_argument('--multi-scale', default=False, help='save to project/name')
