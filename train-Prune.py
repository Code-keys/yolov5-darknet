    ...
    def updateBN(sr_flag, model, s ):
        if sr_flag:
            for k, v in model.named_modules():  
                if ".cv2.bn" in k : 
                    v.weight.grad.data.add_(s * torch.sign(v.weight.data))  # L1
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                BNOptimizer.updateBN( epoch > 5, model, opt.s ) 
...
def prune( pt, cfg, cut_ratio ):

    model = attempt_load( pt )

    # ... spare training is OK
...
if __name__ == '__main__':
    ...
    parser.add_argument('--s', type=float, default=0.0001, help='spare training ratio')
